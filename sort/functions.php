...
function do_sorting($args){
    ...
    echo("<h1>./sort.exe $args out.txt 2>log.txt</h1>");
    echo("<br>");
	...
	while(1){
		...
		$fh = fopen($marker,'w');
		system("./sort.exe $args out.txt 2>log.txt");        // do system command;
		unlink($marker);
		break;
		}else{
		echo '.';continue;
	...
	if(filesize('log.txt')>0){
		...
		unlink('log.txt');
		    ...
		    echo("<h1 id='output'><a id='output' href='out.txt'>output matrix</a></h1>");
