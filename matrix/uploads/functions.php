...
function do_matrix_add($args){
    ...
    echo("<h1>./addition.exe $args out.txt 2>log.txt</h1>");
    echo("<br>");
	...
	while(1){
		...
		$fh = fopen($marker,'w');
		system("./addition.exe $args outadd.txt 2>log.txt");        // do system command;
		unlink($marker);
		break;
		}else{
		echo '.';continue;
	...
	if(filesize('log.txt')>0){
		...
		unlink('log.txt');
		    ...
		    echo("<h1 id='output'><a id='output' href='outadd.txt'>output matrix</a></h1>");
